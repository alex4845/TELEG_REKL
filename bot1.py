    ...
    if mess1 == "Получить 10 последних креативов":
        ...
        for post in r:
            try:
                ...
                await message.answer_photo(photo=screenshot_entity, caption=post[1] + '\nКуда ведет: ' + post[2],
...
async def interval():
    t_n = datetime.now()
    current_time = datetime.now().replace(hour=13, minute=00, second=0)
    current_time1 = datetime.now().replace(hour=13, minute=13, second=0)
    if t_n > current_time and t_n < current_time1:
        r = await read_from_base(50)
        c = 0
        for post in r:
            c += 1
            screenshot_data, screenshot_name = post[-1], str(post[0]) + '.png'
            screenshot_entity = types.InputFile(io.BytesIO(screenshot_data), filename=screenshot_name)
            await bot.send_photo(chat_id=-1002018831317, photo=screenshot_entity, caption=post[1] +
                                '\nКуда ведет: ' + post[2], parse_mode=ParseMode.HTML)
            if c == 20:
                time.sleep(61)
                c = 0
